...
class Platform:
    ...
    def _init_validate_class_attributes(cls, serialized: dict, section: str, configfile: str = None) -> dict:
        """_init_validate_class_attributes      Initialize and validate the Platform definition (file configuration and serialized)
        ...
        Returns:
            new_serialized {dict}       Platform defintion
        """
        if not configfile:
        ...
        for k in serialized.keys():
            ...
            if keyname in Platform._SERIALIZED_FIELDS:
                # String definition
        ...
        # new_serialized.setdefault("xxx", PLatform._PLATFORM_DEFAULT_XXX)
    ...
    async def deactivate_target_platform(self, pfid: int):
        """
        Deactivate target platform using Unique ID (integer)
        You can get ID using get_target_platform_details
